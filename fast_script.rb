...
File.open(file).each do |line|
  ...
  valid = 0 if info[2].to_i < 10
  valid = 0 if info[3].to_f < 30.0

  # Compare it to control

  info_control = line[control_number].split(":")

  valid = 0 if info_control[0] =~ /1\/1/ && !(info[0] =~ /2\/2/) && info[3].to_f >= 20.0
  valid = 0 if info_control[0] == info[0] &&  info[3].to_f >= 20.0
  ...
  puts "#{line[0]}\t#{line[1]}\t#{num_mut_reads.to_i}\t#{num_all_reads.to_i}\t#{info[3]}\t#{valid}"
  #puts info.join(":")
